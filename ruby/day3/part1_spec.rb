...
describe "Part1" do
  ...
  describe "#find_numbers" do
    it "finds the numbers" do
      input = "467..114.....*........35..633.......#...617*...........+.58...592...........755....$.*.....664.598.."
      expect(find_numbers(input)).to match_array(
        [
          ["467", 0, 2],
          ["114", 5, 7],
          ["35", 22, 23],
  ...
  describe "#adjacent?" do
    it "returns true when symbol is adjacent to the number" do
      ...
      symbol = 36
      expect(adjacent?(symbol, number, width, positions)).to eq(true)
    end
    ...
    it "returns false when symbol is not adjacent to the number" do
      ...
      positions = [-(width + 1), -width, -(width - 1), -1, 1, width - 1, width, width + 1]
      symbol = 55
      expect(adjacent?(symbol, number, width, positions)).to eq(false)
    end
